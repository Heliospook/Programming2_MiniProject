...
Seller022::Seller022(string id):Seller(id){
    ...
    for(int id=200;id<210;id++){
        products[Globals::Category(1)][to_string(id)] = new Mobile(
            to_string(id),
            companies[rand()%4] + " " + char(57 + rand()%26) + to_string(1 + rand()%10),
            8000 + (rand()%10000000)/100,
            100 + rand()%200
        );
    }
};
...
bool Seller022::buy_product(string product_id, int quantity) {
    ...
    Products newProd = prod;
    newProd.set_quantity(prod.get_quantity() - quantity);
    newProd.set_price(prod.get_price()*1.005*quantity);
    return true;
}